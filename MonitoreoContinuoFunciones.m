%% SCRIPT PRINCIPAL - MonitoreoContinuoFunciones.m
% Guarda este archivo COMPLETO como "MonitoreoContinuoFunciones.m"
clear; clc; close all;

% === FUNCIONES PRINCIPALES ===
function [waveformsAll, captureTimes] = captureWaveforms(rx, monitorTime, interval, framesPerCapture, sampleRate)
    captureDuration = seconds((framesPerCapture+1)*10e-3);
    numCaptures = floor(monitorTime/interval);
    waveformsAll = cell(numCaptures,1);
    captureTimes = zeros(numCaptures,1);
    
    fprintf('Capturando %d waveforms...\n', numCaptures);
    tic;
    for k = 1:numCaptures
        t_start = tic;
        waveformsAll{k} = capture(rx, captureDuration);
        captureTimes(k) = toc(t_start);
        fprintf('[Captura %d] Tiempo: %.3fs\n', k, captureTimes(k));
        if k < numCaptures
            pause(interval);
        end
    end
...
function [resourceGrids, demodTimes, gridSSB1All, ssbTimes, powerVec, snrVec, cellIDVec, ssbRects, ssbTexts] = demodulateAll(waveformsAll, centerFrequency, scs, sampleRate)
    ...
    for k = 1:numCaptures
        ...
        fprintf('[Demod %d] Tiempo: %.3fs | Pot=%.1fdB | SNR=%.1fdB | cellID=%d\n', ...
...
% =====================================================
% EJECUCIÓN PRINCIPAL - Pega todo esto al final del archivo
% =====================================================
fprintf('=== INICIANDO MONITOREO CONTINUO ===\n');
...
%% PASO 1: Captura
[waveformsAll, captureTimes] = captureWaveforms(rx, monitorTime, interval, framesPerCapture, rx.SampleRate);

%% PASO 2: Demodulación
[resourceGrids, demodTimes, gridSSB1All, ssbTimes, powerVec, snrVec, cellIDVec, ssbRects, ssbTexts] = ...
    demodulateAll(waveformsAll, rx.CenterFrequency, scs, rx.SampleRate);

%% PASO 3: Visualización
gridTimes = generateResourceGrids(resourceGrids, ssbTimes, rx.CenterFrequency, ssbRects, ssbTexts);

%% REPORTE FINAL
fprintf('\n=== RESUMEN DE TIEMPOS ===\n');
fprintf('Captura promedio: %.3fs (total: %.3fs)\n', mean(captureTimes), sum(captureTimes));
fprintf('Demodulación promedio: %.3fs (total: %.3fs)\n', mean(demodTimes), sum(demodTimes));
fprintf('Generación Grid: %.3fs\n', gridTimes);
