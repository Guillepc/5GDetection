function [waveformsAll, resourceGrids, ssbTimes, powerVec, snrVec, cellIDVec] = MonitoreoContinuo2Mod
  ...
  scs = "30kHz";
  nrbDemod = 30; % ampliar para capturar más info, limitar para overflow
  scsNumeric = double(extract(scs, digitsPattern));
  ofdmInfo = nrOFDMInfo(nrbDemod, scsNumeric);
  ...
  for k = 1:numCaptures
      ...
      % Detecta SSB, pero no modifica la visualizacion
      ...
      % Demodula el resource grid (ejemplo: 30 RBs, 4 símbolos)
      nSlot = 0;
      fullGrid = nrOFDMDemodulate(waveform, nrbDemod, scsNumeric, nSlot, "SampleRate", rx.SampleRate);

      % Opcional extraer sólo ciertos símbolos (ej. 2 a 5) para la visualización
      % fullGridSlice = fullGrid(:,2:5,1);

      resourceGrids{k} = fullGrid; % Guardar grid completo
  ...
  % Visualización interactiva de los resource grids capturados
  visualizeResourceGridsOverlay(resourceGrids, ssbTimes, rx.CenterFrequency);
end


% Función simplificada para detección SSB
...
% Función para visualizar resource grids (todos los capturados)
function visualizeResourceGridsOverlay(resourceGrids, ssbTimes, freqCenter)
  ...
  for k = 1:numCaptures
      ...
      allGrids(:, :, k) = abs(grid(:, :, 1)); % considera el primer antena o primer slot
  end
...
% Función para estimar SNR básica
